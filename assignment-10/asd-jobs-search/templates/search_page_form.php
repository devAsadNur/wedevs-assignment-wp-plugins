...
<div class="job-search-form-wrapper">
    <form action="" method="get" id="job-search-form">
        ...
        <?php $job_fulltime = isset( $_GET['job-fulltime'] ) ? $_GET['job-fulltime'] : '' ?>
        <input type="checkbox" name="job-fulltime" id="job-fulltime" value="on" <?php checked( $job_fulltime, 'on' ); ?>>
        <label for="job-fulltime"><?php esc_html_e( 'Full time only', 'asd-jobs-search' ); ?></label>

        <input type="submit" name="job-search-submit" value="<?php esc_attr_e( 'Search Jobs', 'asd-jobs-search' ); ?>">
