...
class Shortcode {
    ...
    public function render_shortcode_form() {
         ...
         */
        ob_start();
        require_once ASD_BOOK_REVIEW_PRO_PATH . "/templates/shortcode_search_form.php";
        echo ob_get_clean();
    ...
    public function post_meta_query_handler( $query_args) {
        ...
        if ( $book_meta_query->have_posts() ) {
            while ( $book_meta_query->have_posts() ) {
                 ...
                 */
                ob_start();
                include ASD_BOOK_REVIEW_PRO_PATH . "/templates/shortcode_search_result_viewer.php";
                echo ob_get_clean();
            }
        } else {
            echo __( 'No book review matched with your query!', 'asd-book-review-pro' );
        }
