...
class Ajax {
    ...
    public function book_rating_request_handler() {
        if ( ! wp_verify_nonce( $_REQUEST['_ajax_nonce'], 'book-review-nonce' ) ) {
            wp_send_json_error( [
                'message' => __( 'Nonce verification failed!', 'asd-book-review-pp' ),
            ] );
        }

        if ( ! isset( $_REQUEST['rating'] ) ) {
            wp_send_json_error( [
                'message' => __( 'Rating can\'t be empty!', 'asd-book-review-pp' ),
            ] );
        }

        if ( ! isset( $_REQUEST['post_id'] ) ) {
            wp_send_json_error( [
                'message' => __( 'Post ID can\'t be empty!', 'asd-book-review-pp' ),
            ] );
        }
        ...
        if ( '' !== $_REQUEST['rating_id'] ) {
            ...
            wp_send_json_success( [
                'message' => __( 'Rating updated successfully!', 'asd-book-review-pp' ),
            ] );
        } else {
            $insert_id = asd_br_insert_rating( $args );

            wp_send_json_success( [
                'message' => __( 'Rating added successfully!', 'asd-book-review-pp' ),
                'rating_id' => $insert_id,
            ] );
        }

        wp_send_json_error( [
            'message' => __(  'Request failed!', 'asd-book-review-pp' ),
        ] );
    }
    ...
    public function book_rating_frontend_handler() {
        wp_send_json_error( [
            'message' => __( 'Please login in first in order to give rating!', 'asd-book-review-pp' ),
        ] );
    }
}
